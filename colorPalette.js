...
var ColorPalette = class ColorPalette {
    ...
    extractColorsFromImage(pictureUri, maxColors = 8, preferLight = null, forceExtraction = false) {
        ...
        try {
            ...
            this._logger.info(`🖼️  Loaded scaled pixbuf: ${pixbuf.get_width()}x${pixbuf.get_height()} = ${pixbufSize.toFixed(1)}MB uncompressed`);
            ...
            try {
                imports.system.gc(); // Force garbage collection of native objects
                ...
                const timerId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 100, () => {
                    ...
                    this._logger.info(`📊 Memory AFTER GC: ${memAfter}MB (Δ ${delta >= 0 ? '+' : ''}${delta.toFixed(1)}MB)`);
                    return GLib.SOURCE_REMOVE; // One-shot timeout
                });
                this._pendingGcTimers.add(timerId); // Track timer for cleanup

                this._logger.debug(`♻️  Triggered GC after wallpaper extraction (freed ~${pixbufSize.toFixed(1)}MB pixbuf memory)`);
            } catch (e) {
            ...
            if (pixbuf) {
                try {
                    // CRITICAL WORKAROUND for GNOME Shell 43-44 GdkPixbuf memory leak:
                    // GdkPixbuf.Pixbuf has a known memory leak in GNOME Shell context where
                    // texture buffers are not properly freed by garbage collector even after
                    // object goes out of scope. Without explicit run_dispose(), each wallpaper
                    // analysis can leak 2-5MB of uncompressed RGB pixel data that persists
                    // until Shell restart. This is a well-documented issue affecting all
                    // GNOME Shell extensions that use GdkPixbuf for image processing.
                    // Real-world impact: Users changing wallpapers 10+ times can experience
                    // 50MB+ memory growth without this explicit cleanup.
                    // See: https://gitlab.gnome.org/GNOME/gdk-pixbuf/-/issues/
                    pixbuf.run_dispose();
            ...
            if (inputStream) {
                try {
                    // Dispose input stream to prevent file descriptor leak.
                    // GNOME Shell has limited file descriptor pool (~1024 on most systems).
                    // Without explicit disposal, repeated wallpaper analysis can exhaust
                    // available descriptors, causing "Too many open files" errors.
    ...
    analyzePixbuf(pixbuf, maxColors, preferLight = false) {
        ...
        this._logger.info(`Analyzed ${processedPixels} pixels (${preferLight ? 'light' : 'dark'} mode), skipped ${skippedTransparent} transparent, ${skippedBlackWhite} out-of-range`);
    ...
    applyColorsToSettings(settings, colorScheme) {
        try {
            ...
            const panelColor = ThemeUtils.rgbaToCss(
            ...
            const popupColor = ThemeUtils.rgbaToCss(
                ...colorScheme.accent,
                menuOpacity
            );
            ...
            if (fullAutoMode) {
                ...
                this._logger.info(`  Wallpaper border accent (calculated, not applied): RGB(${borderAccent.join(', ')})`);
    ...
    setupBackgroundMonitoring(settings, bgSettings, onColorsChanged, overlayThemeManager = null) {
        ...
        if (this._monitorIds.length > 0) {
            this._logger.warn(`Wallpaper monitoring already active (${this._monitorIds.length} monitors), skipping setup`);
            return;
        }
        ...
        if (overlayThemeManager && this._interfaceSettings) {
            ...
            this._logger.info(`Color-scheme monitoring disabled (overlayThemeManager: ${!!overlayThemeManager}, interfaceSettings: ${!!this._interfaceSettings})`);
        }

        this._logger.info(`Background monitoring enabled for: ${activeUriKey} (${isDarkPreferred ? 'dark' : 'light'} mode)`);
    ...
    handleBackgroundChange(settings, bgSettings, onColorsChanged) {
        try {
            ...
            this._logger.info(`Color extraction mode: ${preferLight ? 'light' : 'dark'} colors (theme brightness preference)`);
            ...
            this._logger.info(`Auto-extracted ${palette.length} ${preferLight ? 'light' : 'dark'} colors from "${wallpaperName}"`);
    ...
    _reconnectBackgroundMonitoring(settings, bgSettings, onColorsChanged, overlayThemeManager) {
        ...
        this._monitorIds = this._monitorIds.filter(monitor => {
            if (monitor.isDark !== null) { // Background URI monitor
                monitor.settings.disconnect(monitor.id);
        ...
        this._logger.info(`Switching monitoring from "${oldPreferDark ? 'picture-uri-dark' : 'picture-uri'}" to "${activeUriKey}"`);
        ...
        this._logger.info(`Background monitoring switched to: ${activeUriKey} (${isDarkPreferred ? 'dark' : 'light'} mode)`);
        ...
        if (newPictureUri && newPictureUri !== 'none') {
            ...
            this._logger.info(`Triggering immediate processing for "${wallpaperName}" (new ${isDarkPreferred ? 'dark' : 'light'} mode)`);
            this.handleBackgroundChange(settings, bgSettings, onColorsChanged);
        } else {
    ...
    extractFromCurrentBackground(forceExtraction = false) {
        try {
            ...
            this._logger.info(`Extracted ${palette.length} ${preferLight ? 'light' : 'dark'} colors for ${preferLight ? 'Light' : 'Dark'} theme`);
    ...
    getPalettePreview(palette, maxColors = 6) {
        ...
        return palette
            .slice(0, maxColors)
            .map(color => ThemeUtils.rgbaToCss(...color, 1.0));
    }
        ...
        try {
            ...
            for (const entry of cacheData.entries || []) {
                ...
                if (age > staleThreshold) {
                    this._logger.debug(`Skipping stale cache entry: ${entry.wallpaperUri} (${Math.floor(age / (24 * 60 * 60 * 1000))} days old)`);
                    continue;
                }
            ...
            this._logger.info(`✅ Loaded ${loadedCount} wallpapers from persistent cache (${cacheData.entries?.length || 0} total entries)`);
    ...
    _schedulePersistentCacheSave() {
        ...
        this._cacheSaveTimer = GLib.timeout_add(
            GLib.PRIORITY_LOW,
            Constants.CACHE_PERSISTENCE.saveDebounceMs,
            () => {
