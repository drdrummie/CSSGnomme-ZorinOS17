...
const { CSSTemplates } = Me.imports.cssTemplates;

var OverlayThemeManager = class OverlayThemeManager {
    ...
    _getShellSettings() {
        if (!this._shellSettings) {
            ...
            this._shellSettings = ExtensionUtils.getSettings(shellThemeSchema);
        }
    ...
    createOverlayTheme(sourceThemeName, settings, interfaceSettings = null) {
        ...
        if (metadata && metadata[`X-${this.extensionName}-Extension`]) {
            ...
            if (previousSourceTheme && previousSourceTheme !== sourceThemeName) {
                this._logger.info(`🔄 Source theme changed (${previousSourceTheme} → ${sourceThemeName}) - clearing base theme cache`);
        ...
        if (interfaceSettings) {
            ...
            try {
                const shellThemeSettings = new imports.gi.Gio.Settings({
                ...
                if (currentShellTheme === this.overlayName) {
                    ...
                    if (metadata && metadata[`X-${this.extensionName}-Extension`]) {
                        ...
                        this._logger.warn(` Overlay Shell theme active but no metadata - using sourceTheme as fallback`);
                    }
                } else {
                ...
                this._logger.warn(` user-theme extension not available, using GTK theme as Shell theme: ${originalShellTheme}`);
            }
        } else {
        ...
        try {
            ...
            this._logger.info(`📊 Memory BEFORE CSS cleanup: ${memBefore}MB (${cssFileCount} files, ${cssSizeMB}MB CSS data)`);
            ...
            try {
                ...
                GLib.timeout_add(GLib.PRIORITY_DEFAULT, 100, () => {
                    ...
                    this._logger.info(`📊 Memory AFTER CSS cleanup + GC: ${memAfter}MB (Δ ${delta >= 0 ? '+' : ''}${delta.toFixed(1)}MB)`);
...
`;
    ...
    detectAndApplyAccentColor(sourcePath, settings, isLightTheme = null) {
        try {
            ...
            const borderAlpha = themeIsLight ? Constants.ACCENT_COLOR_ALPHA.border.lightTheme : Constants.ACCENT_COLOR_ALPHA.border.darkTheme;
            const bgAlpha = themeIsLight ? Constants.ACCENT_COLOR_ALPHA.background.lightTheme : Constants.ACCENT_COLOR_ALPHA.background.darkTheme;
...
`;
    ...
    _extractShellCssVars(sourcePath, settings, isLightTheme = null) {
        ...
        const backdropFilter = this._cssTemplates.getBackdropFilter(
            blurRadius, blurSaturate, blurContrast, blurBrightness
        );
        ...
        const hoverOpacity = isZorinTheme ? (themeIsLight ? Constants.ZORIN_HOVER_OPACITY.lightTheme : Constants.ZORIN_HOVER_OPACITY.darkTheme) : Constants.ZORIN_HOVER_OPACITY.lightTheme; // Default fallback
        const activeOpacity = isZorinTheme ? (themeIsLight ? Constants.ZORIN_ACTIVE_OPACITY.lightTheme : Constants.ZORIN_ACTIVE_OPACITY.darkTheme) : Constants.ZORIN_ACTIVE_OPACITY.lightTheme; // Default fallback
        ...
        const shadowColor = (shadowColorSetting && shadowColorSetting !== 'rgba(255, 255, 255, 0.7)' && shadowColorSetting !== 'rgba(0, 0, 0, 0.7)')
            ? shadowColorSetting
    ...
    _enforceCacheLRU(cache, maxSize, cacheName) {
        if (cache.size > maxSize) {
            ...
            if (typeof evictedValue === 'string') {
                this._logger.debug(`  🗑️  Evicted oldest ${cacheName}: ${firstKey} (${(evictedValue.length / 1024).toFixed(1)}KB)`);
            } else {
    ...
    _createComponentCacheKey(componentName, vars) {
        ...
        const relevantVars = {
            panel: [
                vars.borderRadius, vars.borderWidth, vars.applyPanelRadius,
                vars.enableZorinIntegration, vars.isZorinTheme, vars.isLightTheme,
                vars.panelBackgroundCss, vars.backdropFilter, vars.shadowColor,
                vars.borderColor, vars.hoverRgb, vars.hoverOpacity, vars.activeOpacity
            ],
            popup: [
                vars.borderRadius, vars.borderWidth, vars.popupBackgroundCss,
                vars.backdropFilter, vars.blurBackgroundOverlay, vars.shadowColor,
                vars.previewBackgroundCss, vars.enableZorinIntegration
            ],
            zorin: [
                vars.borderRadius, vars.sourceThemeName, vars.enableZorinIntegration,
                vars.isZorinTheme, vars.accentRgb, vars.hoverRgb, vars.activeOpacity
            ],
            fluent: [
                vars.sourceThemeName, vars.isZorinTheme, vars.enableZorinIntegration,
                vars.borderRadius, vars.accentRgb
            ]
        };
...
`;
    ...
    updateOverlayCss(settings, detectAccentColor = false, interfaceSettings = null) {
        ...
        try {
            ...
            const hitRate = this._baseThemeCacheStats.hits + this._baseThemeCacheStats.misses > 0
                ? ((this._baseThemeCacheStats.hits / (this._baseThemeCacheStats.hits + this._baseThemeCacheStats.misses)) * 100).toFixed(1)
                : 0;
            this._logger.info(`📊 Base Theme Cache Stats: ${this._baseThemeCacheStats.hits} hits, ${this._baseThemeCacheStats.misses} misses (${hitRate}% hit rate)`);

            // Log component cache statistics
            const componentHitRate = this._componentCacheStats.hits + this._componentCacheStats.misses > 0
                ? ((this._componentCacheStats.hits / (this._componentCacheStats.hits + this._componentCacheStats.misses)) * 100).toFixed(1)
                : 0;
            this._logger.info(`📊 Component Cache Stats: ${this._componentCacheStats.hits} hits, ${this._componentCacheStats.misses} misses (${componentHitRate}% hit rate)`);
    ...
    _updateShellCss(sourcePath, shellInfo, settings, isLightTheme = null) {
        ...
        if (shellInfo.hasShellCss) {
            ...
            // Force Shell theme reload to bypass CSS cache
            this._forceShellThemeReload();
        }
    ...
    destroy() {
        ...
        if (this._baseThemeCache) {
            ...
            this._baseThemeCache.clear();
            this._logger.info(`🗑️  Cleared base theme cache: ${cacheSize} entries (${(totalMemory / 1024).toFixed(1)}KB freed)`);
        }
        ...
        if (this._componentCssCache) {
            ...
            this._logger.info(`🗑️  Cleared component CSS cache: ${cacheSize} entries (${(totalMemory / 1024).toFixed(1)}KB freed)`);
        }
